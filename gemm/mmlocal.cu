...
#define TILE 16               // same tile size the kernel expects
...
int index_to_position(const int idx[3], const int strides[3], int dims) {
  int pos = 0;
  #pragma unroll
  for (int d = 0; d < dims; ++d) pos += idx[d] * strides[d];
  return pos;
}
 ...
 */
  ...
  __shared__ float a_shared[TILE][TILE+1];
  __shared__ float b_shared[TILE][TILE+1];
  ...
  for (int k = 0; k < a_local_shape[2]; k+=TILE)
  {
      ...
      {
          int linearAIndex = index_to_position(aIndex, a_local_strides, 3);
...
int main() {
  ...
  for (size_t i = 0; i < (bytesA/sizeof(float)); ++i) hA[i] = 1.f;
  for (size_t i = 0; i < (bytesB/sizeof(float)); ++i) hB[i] = 1.f;
  ...
  CHECK(cudaFree(d_a_shape)); CHECK(cudaFree(d_b_shape)); CHECK(cudaFree(d_out_shape));
  CHECK(cudaFree(d_a_strides)); CHECK(cudaFree(d_b_strides)); CHECK(cudaFree(d_out_strides));
